...
session_start();
    ...
    if ($users) {
      foreach ($users as $row) {
        $output .= '<tr>
                      ...
                      <td>
                      <a href="#" id="' . $row['id_projet'] . '" class="btn btn-success btn-sm rounded-pill py-0 editLink" data-toggle="modal" data-target="#editUserModal">Edit</a>
  ...
  if (isset($_POST['update'])) {
    ...
    if ($db->update($id, $titre, $github, $lien, $image)) {
      echo $util->showMessage('success', 'Projet mis à jour avec succès!');
    } else {
      echo $util->showMessage('danger', 'Something went wrong!');
    }
  }
  ...
  if (isset($_GET['delete'])) {
    $id = $_GET['id'];
    if ($db->delete($id)) {
      echo $util->showMessage('info', 'Projet supprimer avec succès!');
    } else {
      echo $util->showMessage('danger', 'Something went wrong!');
    }
  }
